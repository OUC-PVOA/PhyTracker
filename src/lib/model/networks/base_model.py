...
class BaseModel(nn.Module):
    ...
    def TraDeS(self, cur_feat, pre_img, pre_hm, addtional_pre_imgs, addtional_pre_hms, inference_feats):
        ...
        if self.opt.inference:
            ...
            while len(feat_list) < self.opt.clip_len:
                feat_list.append(cur_feat[0])
    ...
    def CVA_MFW(self, feat_list, support_feats):
        ...
        adaptive_weights = torch.split(adaptive_weights, 1, dim=1)
    ...
    def CVA(self, embedding_prime, feat_prev, batch_size, h_c, w_c):
        ...
        c = torch.matmul(embedding_prime, _embedding_prev)
        c = c.view(batch_size, h_c * w_c, h_c, w_c)

        c_h = c.max(dim=3)[0]
        c_w = c.max(dim=2)[0]
        c_h_softmax = F.softmax(c_h * self.tempature, dim=2)
        c_w_softmax = F.softmax(c_w * self.tempature, dim=2)
        v = torch.tensor(self.v, device=self.opt.device)
    ...
    def MFW(self, support_feat, tracking_offset, feat_cur, feat_prev, batch_size, h_f, w_f):
        ...
        if memory_offset.numel() > 0:
            phy_tracking_offset = tracking_offset - memory_offset.detach()
            memory_offset = ((memory_offset + tracking_offset) / 2).detach()
        else:
            phy_tracking_offset = tracking_offset
            memory_offset = tracking_offset.detach()
        tracking_offset = tracking_offset + phy_tracking_offset
    ...
    def gamma(self, tracking_offset, feat_cur, feat_prev, batch_size, h_f, w_f):
        ...
        off_h_deform = self.conv_offset_h(feat_offs_h)[:, :, None, :, :]
        off_w_deform = self.conv_offset_w(feat_offs_w)[:, :, None, :, :]
        off_deform = torch.cat((off_h_deform, off_w_deform), dim=2)
