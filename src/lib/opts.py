...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--test_dataset', default='',
                             help='coco | kitti | coco_hp | pascal | PMOT2023')
    ...
    self.parser.add_argument('--demo', default='',
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default='../models/model.pth',
    ...
    self.parser.add_argument('--trades', action='store_true', help='')
    ...
    self.parser.add_argument('--save_point', type=str, default='',
                             help='when to save the model to disk.')
    self.parser.add_argument('--num_epochs', type=int, default=60,
                             help='total training epochs.')
    self.parser.add_argument('--batch_size', type=int, default='',
                             help='batch size')
    self.parser.add_argument('--master_batch_size', type=int, default=-1,
                             help='batch size on the master gpu.')
    self.parser.add_argument('--num_iters', type=int, default=-1,
                             help='default: #samples / batch_size.')
    self.parser.add_argument('--val_intervals', type=int, default='',
  ...
  def parse(self, args=''):
    ...
    if opt.resume and opt.load_model == '':
      opt.load_model = os.path.join(opt.save_dir, 'model.pth')
    return opt
  ...
  def init(self, args=''):
    default_dataset_info = {
        'ctdet': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'},
        'multi_pose': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'},
        'ddd': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'},
        'tracking,ctdet': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'},
        'tracking,multi_pose': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'},
        'tracking,ddd': {'default_resolution': [1024, 750], 'num_classes': 21,
                'mean': [1.816706, 1.856168, 1.751525], 'std': [2.632229, 2.685731, 2.541777],
                'dataset': 'PMOT2023'}
    }
    opt = self.parse()
